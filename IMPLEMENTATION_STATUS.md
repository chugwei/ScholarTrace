...
- [ ] M7.2 Run Manifest、现有产物导入和不可验证状态（当前）
- [ ] M7.3 指标独立重算、统计汇总和 Claim 更新
...
- M6.1–M6.3 已实现 AlgorithmSpec/PriorArtMap、InnovationCandidate、方法差异、完整度排序、证伪提案和验证入口门禁；`approved_for_experiment` 仍不是创新结论。M7.1 已实现计划冻结，实验、论文、Web 和部署能力仍未实现。

## 下一步

下一步实现 M7.2 Run Manifest、现有日志/配置/权重/指标导入和不可验证状态；不得把训练日志临时值当作最终指标。
