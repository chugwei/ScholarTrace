...
- M6.1–M6.3 已实现 AlgorithmSpec/PriorArtMap、InnovationCandidate、方法差异、完整度排序、证伪提案和验证入口门禁；`approved_for_experiment` 仍不是创新结论。M7.1–M7.3 已实现计划冻结、Run/Metric 导入边界、独立重算、verified 聚合和 Claim 降级。M8 已发布 v0.8.0，包含 frozen plan 绑定、argv 安全策略、异步本地进程、取消/超时、日志事件、成功后 staging 发布、DebugCase 证据链、隔离回归、JSON/DVC 离线适配和 MLflow 可用性降级。M9.1–M9.3 已完成 FigureSpec 门禁、可重建三格式 Bundle、verified-only 建议、来源 Caption、数值和 provenance 检查；视觉验收、Docker 真实运行、MLflow 在线服务、论文、Web 和部署能力仍未实现。

## 下一步

下一步完成 M9.4 实际打开 PNG/SVG/PDF 做视觉验收、独立发布候选和 `v0.9.0`；不把失败日志或临时指标升级为科研 Claim。
