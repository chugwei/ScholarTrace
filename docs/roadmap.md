...
| 里程碑 | 版本 | 状态 | 分支 | Tag | 验证 |
|---|---|---|---|---|---|
| M0 仓库骨架 | v0.0.1 | blocked | main | 未创建 | 本地通过，远端待验证 |
...
| M0.4 | CI 与质量门禁 | 本地和 GitHub Actions 执行同一锁定检查 | blocked |
...
1. 许可证类型尚未确认。
2. `main` 尚未完成远端同步和 Actions 验证。

恢复顺序：确认许可证、同步 `main`、核验远端 ref 与 Actions，再创建版本 Tag。
